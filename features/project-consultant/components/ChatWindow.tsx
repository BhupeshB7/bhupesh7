...
import { ConsultantSession, LeadData } from "@/lib/types";
...
function LandingState({ onStart }: { onStart: (msg: string) => void }) {
  ...
  return (
    <div className="flex flex-col items-center gap-5 py-6 px-2 text-center">
      ...
      >
        <div
          className="flex items-end gap-2 px-3.5 py-2.5 rounded-xl"
          style={{
        ...
        >
          <textarea
            ...
            className="flex-1 text-[12px] bg-transparent border-none outline-none resize-none leading-relaxed"
            style={{
        ...
        >
          No technical knowledge needed · Enter to send · Shift+Enter for new
          line
        </p>
      </motion.div>
    </div>
...
function AutoTextarea({
  ...
  return (
    <div
      className="flex items-end gap-2 px-3.5 py-2.5 rounded-xl transition-all duration-200"
      style={{
        background: theme.bg.base,
        border: `1px solid ${disabled ? theme.border.subtle : theme.border.default}`,
      }}
    >
      <textarea
        ref={ref}
      ...
      <button
        ...
        className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-200 mb-0.5"
        style={{
          background: value.trim() && !disabled ? c : theme.surface[2],
          border: "none",
    ...
    <div className="flex flex-col gap-3 h-full">
      {(session.phase === "discovery" || session.phase === "brief-ready") && (
        <>
          ...
          {error && (
            <p
              className="text-[11px] text-center flex-shrink-0"
          ...
          {session.phase === "brief-ready" && !isTyping && (
            <motion.button
              ...
              className="w-full py-2.5 rounded-xl text-[12px] font-semibold transition-all duration-200 flex-shrink-0"
              style={{
                background: c,
