...
import ProjectConsultant from "@/features/project-consultant/components/ProjectConsultant";
...
function AudienceBg() {
  const spokes = [
    { angle: 0, x2: "1120", y2: "350" },
    { angle: 30, x2: "1050.333", y2: "610" },
    { angle: 60, x2: "860", y2: "800.333" },
    { angle: 90, x2: "600", y2: "870" },
    { angle: 120, x2: "340", y2: "800.333" },
    { angle: 150, x2: "149.667", y2: "610" },
    { angle: 180, x2: "80", y2: "350" },
    { angle: 210, x2: "149.667", y2: "90" },
    { angle: 240, x2: "340", y2: "-100.333" },
    { angle: 270, x2: "600", y2: "-170" },
    { angle: 300, x2: "860", y2: "-100.333" },
    { angle: 330, x2: "1050.333", y2: "90" },
  ];
  return (
    <>
      <div
...
function HeroDesignLayer() {
  return (
    <motion.svg
      className="absolute right-[-180px] top-1/2 hidden h-[680px] w-[760px] -translate-y-1/2 lg:block pointer-events-none"
      viewBox="0 0 760 680"
      fill="none"
      aria-hidden="true"
      initial={{ opacity: 0, x: 24 }}
      animate={{ opacity: 1, x: 0 }}
      transition={{ duration: 0.8, delay: 0.2, ease: "easeOut" } as Transition}
    >
      <defs>
        <linearGradient id="hero-flow" x1="70" y1="92" x2="610" y2="544">
          <stop stopColor={c} stopOpacity="0.34" />
          <stop
            offset="0.54"
            stopColor={theme.accent.primaryHover}
            stopOpacity="0.14"
          />
          <stop offset="1" stopColor={c} stopOpacity="0" />
        </linearGradient>
        <radialGradient id="hero-node" cx="50%" cy="50%" r="50%">
          <stop stopColor={c} stopOpacity="0.5" />
          <stop offset="1" stopColor={c} stopOpacity="0" />
        </radialGradient>
        <filter
          id="hero-soft-glow"
      ...
      <path
        d="M80 430C164 260 302 174 494 172C594 171 662 126 704 48"
        stroke="url(#hero-flow)"
        strokeWidth="1.4"
      />
      <path
        d="M124 536C230 408 360 350 514 362C598 369 664 331 714 248"
        stroke={`${c}16`}
        strokeWidth="1"
      />
      <path
        d="M42 286C168 330 279 317 375 247C453 190 540 180 636 218"
        stroke={`${c}12`}
        strokeWidth="1"
      />
      ...
      {[120, 200, 280, 360, 440].map((x, i) => (
        <path
          key={x}
          d={`M${x} 76V606`}
          stroke={i % 2 === 0 ? `${c}08` : "rgba(255,255,255,0.035)"}
          strokeWidth="1"
        />
      ))}
      {[120, 200, 280, 360, 440, 520].map((y, i) => (
        <path
          key={y}
          d={`M32 ${y}H702`}
          stroke={i % 2 === 0 ? "rgba(255,255,255,0.035)" : `${c}07`}
          strokeWidth="1"
        />
      ))}
        ...
        {[
          ...
          <g key={`${node.cx}-${node.cy}`}>
            <circle
              ...
              fill="url(#hero-node)"
            />
            <circle cx={node.cx} cy={node.cy} r={node.r} fill={c} />
          </g>
        ))}
      </g>
      ...
      <rect
        ...
        stroke={theme.accent.border}
      />
      <path
        d="M306 272H416M306 306H460M306 340H392"
        stroke={`${c}55`}
        strokeWidth="3"
        strokeLinecap="round"
      />
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed bottom-0 right-0 native-cursor-zone"
            style={{
              zIndex: 99999,
              width: "clamp(92vw, 92vw, 92vw)",
              ...
              borderRight: "none",
              borderBottom: "none",
              boxShadow: `-8px -8px 48px rgba(0,0,0,0.55), -2px -2px 0 ${theme.border.soft}`,
              display: "flex",
              flexDirection: "column",
              overflow: "hidden",
            }}
            // Desktop override via inline media — handled via CSS class below
          >
            <style>{`
            ...
            >
              <ProjectConsultant onClose={onClose} />
            </div>
          </motion.div>
        </>
      )}
...
function ChatSection({ onOpenChat }: { onOpenChat: () => void }) {
  const headerRef = useRef<HTMLDivElement>(null);
  const headerInView = useInView(headerRef, { once: true, amount: 0.3 });
        ...
        <div className="max-w-2xl">
          <div ref={headerRef} className="flex flex-col gap-6">
            ...
            <motion.div
              ...
              animate={headerInView ? { opacity: 1, y: 0 } : {}}
              transition={
                { duration: 0.5, delay: 0.46, ease: "easeOut" } as Transition
              }
              className="flex justify-start lg:justify-end"
            >
              <button
                onClick={onOpenChat}
                className="group inline-flex items-center gap-3 px-6 py-3.5 rounded-2xl text-sm font-semibold transition-all duration-300"
                style={{
                  background: c,
                  color: theme.accent.primaryForeground,
                  boxShadow: `0 0 32px ${c}35, 0 4px 16px rgba(0,0,0,0.3)`,
                  border: "none",
                  cursor: "pointer",
                  fontFamily: "Syne, sans-serif",
                  fontSize: "14px",
                ...
                onMouseEnter={(e) => {
                  ...
                  (e.currentTarget as HTMLButtonElement).style.boxShadow =
                    `0 0 44px ${c}50, 0 6px 24px rgba(0,0,0,0.35)`;
                ...
                onMouseLeave={(e) => {
                  (e.currentTarget as HTMLButtonElement).style.background = c;
                  (e.currentTarget as HTMLButtonElement).style.boxShadow =
                    `0 0 32px ${c}35, 0 4px 16px rgba(0,0,0,0.3)`;
                  (e.currentTarget as HTMLButtonElement).style.transform =
                    "translateY(0)";
                }}
              >
                ...
                <span>Open AI Project Discovery</span>
                <svg
                  width="14"
                  height="14"
                  viewBox="0 0 16 16"
                  fill="none"
                  style={{ opacity: 0.7, transition: "transform 0.2s" }}
                >
                  <path
...
function FinalCTASection({ onOpenChat }: { onOpenChat: () => void }) {
  ...
  return (
    <>
      <main>
        ...
        <ServicesSection />
        <ChatSection onOpenChat={() => setChatOpen(true)} />
        <AudienceSection />
        <CollaborationSection />
        <ProcessSection />
        <FinalCTASection onOpenChat={() => setChatOpen(true)} />
      </main>
